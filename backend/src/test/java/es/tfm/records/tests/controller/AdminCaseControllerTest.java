...
import es.tfm.records.entrypoints.controller.AdminCaseController;
...
class AdminCaseControllerTest {
    ...
    void getCaseDetail_shouldReturn404WhenNotFound() throws Exception {
        ...
        when(backofficeService.getCaseDetail(eq(caseId)))
                .thenThrow(new es.tfm.records.application.exception.ResourceNotFoundException("PROC", caseId.toString()));
