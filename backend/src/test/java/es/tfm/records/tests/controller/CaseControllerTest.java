...
import es.tfm.records.application.service.CaseService;
import es.tfm.records.application.service.EniPackagerService;
import es.tfm.records.entrypoints.advice.GlobalExceptionHandler;
import es.tfm.records.entrypoints.controller.CaseController;
...
class CaseControllerTest {
    ...
    void listCases_shouldReturn200() throws Exception {
        ...
        CaseItem item = new CaseItem(UUID.randomUUID(), "Test", "DRAFT", Instant.now(), null, "Cat", "Unit", null, null);
    ...
    void createCase_shouldReturn201() throws Exception {
        ...
        CaseItem created = new CaseItem(UUID.randomUUID(), "New Case", "DRAFT", Instant.now(), null, "Cat", "Unit", null, null);
    ...
    void getCaseDetail_shouldReturn200() throws Exception {
        ...
        CaseDetail detail = new CaseDetail(
                ...
                UUID.randomUUID(), Map.of("field", "value"), null, null);
    ...
    void getCaseDetail_shouldReturn404_whenNotFound() throws Exception {
        ...
        when(caseService.getCaseDetail(eq(caseId), eq(ownerId)))
                .thenThrow(new es.tfm.records.application.exception.ResourceNotFoundException("PROCEDURE", caseId.toString()));
    ...
    void getCaseDetail_shouldReturn403_whenNotOwner() throws Exception {
        ...
        UUID caseId = UUID.randomUUID();
        when(caseService.getCaseDetail(eq(caseId), eq(ownerId)))
                .thenThrow(new es.tfm.records.application.exception.AccessDeniedException("PROC", caseId.toString()));
    ...
    void getCaseStatus_shouldReturn200() throws Exception {
        UUID ownerId = UUID.randomUUID();
        UUID caseId = UUID.randomUUID();
        CaseStatusResponse response = new CaseStatusResponse(caseId, "SUBMITTED", Instant.now(), "Document Review", null, null);
    ...
    void requestAmendment_shouldReturn200() throws Exception {
        UUID ownerId = UUID.randomUUID();
        UUID caseId = UUID.randomUUID();
        CaseStatusResponse response = new CaseStatusResponse(caseId, "AMENDMENT_REQUIRED", Instant.now(), "Amend Documents", null, null);
    ...
    void requestAmendment_shouldReturn200_withEmptyBody() throws Exception {
        UUID ownerId = UUID.randomUUID();
        UUID caseId = UUID.randomUUID();
        CaseStatusResponse response = new CaseStatusResponse(caseId, "AMENDMENT_REQUIRED", Instant.now(), null, null, null);
        when(caseService.requestAmendment(eq(caseId), eq(ownerId), any())).thenReturn(response);
    ...
    void submitCase_shouldReturn409_whenNotInDraft() throws Exception {
        ...
        UUID caseId = UUID.randomUUID();
        when(caseService.submitCase(eq(caseId), eq(ownerId)))
                .thenThrow(new es.tfm.records.application.exception.ConflictException("PROC", "Cannot submit a case that is not in DRAFT status. Current: SUBMITTED"));
    ...
    void updateDraft_shouldReturn200() throws Exception {
        ...
        UUID caseId = UUID.randomUUID();
        UUID procedureTypeId = UUID.randomUUID();
        CaseStatusResponse response = new CaseStatusResponse(caseId, "DRAFT", Instant.now(), null, null, null);
    ...
    void updateDraft_shouldReturn409_whenNotInDraft() throws Exception {
        ...
        UUID procedureTypeId = UUID.randomUUID();
        when(caseService.updateDraft(eq(caseId), eq(ownerId), any(CreateCaseRequest.class)))
                .thenThrow(new es.tfm.records.application.exception.ConflictException("PROC", "Case not in DRAFT status"));
