...
import es.tfm.records.application.service.AuthService;
import es.tfm.records.entrypoints.advice.GlobalExceptionHandler;
import es.tfm.records.entrypoints.controller.AuthController;
...
class AuthControllerTest {
    ...
    void refreshToken_shouldReturn401_whenTokenIsExpired() throws Exception {
        when(authService.refreshToken(any(TokenRefreshRequest.class)))
                .thenThrow(new es.tfm.records.application.exception.AuthenticationException("AUTH-401-EXPIRED_TOKEN", "Refresh token expired"));
    ...
    void verifyOtp_shouldReturn400_whenOtpIsExpired() throws Exception {
        doThrow(new es.tfm.records.application.exception.AuthenticationException("AUTH-400-INVALID_OTP", "Invalid or expired OTP"))
                .when(authService).verifyOtp(any(OtpRequest.class));
    ...
    void verifyEmail_shouldReturn401_whenTokenIsInvalid() throws Exception {
        doThrow(new es.tfm.records.application.exception.AuthenticationException("AUTH-401-INVALID_TOKEN", "Invalid or expired verification token"))
    ...
    void getProfile_shouldReturn404_whenUserNotFound() throws Exception {
        ...
        when(authService.getProfile(eq(userId)))
                .thenThrow(new es.tfm.records.application.exception.ResourceNotFoundException("User", userId.toString()));
    ...
    void resetPassword_shouldReturn400_whenTokenIsExpired() throws Exception {
        doThrow(new es.tfm.records.application.exception.AuthenticationException("AUTH-400-EXPIRED_TOKEN", "Reset token expired"))
                .when(authService).resetPassword(any(PasswordResetConfirmRequest.class));
